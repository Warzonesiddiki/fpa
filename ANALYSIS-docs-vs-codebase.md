...
> Quick audit, 2026-08-31 (HISTORICAL snapshot at HEAD 4cc5274 on 2026-09-06: 42 screens / 78 handlers). Purpose: confirm the project is on track to deliver the
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| Product **spec** (docs)                   | **Complete** — 54 locked docs, 38 MVP features, 42 screens, 78 handlers, 97 error codes, 56 DB tables (HISTORICAL HEAD 4cc5274) | Huge but **unlocked / sprawling** (62 docs, 187 engines, 53 pages)                                                                                                        | **Keep current docs as source of truth.** Don't import predecessor docs. |
...
| Screens         | **42** (S-001…S-076) (HISTORICAL HEAD 4cc5274: 42 screens) | **9** (S-001, 002, 004, 010, 020, 021, 022, 023 + first-run-PIN)                                                            | ~**33 screens missing**                                   |
| API commands    | **78 handlers** (HISTORICAL HEAD 4cc5274)                  | **20 Rust handlers / 19 typed TS commands** (session, company, calendar, coa, pack, import.\*, model.cell.set/model.recalc) | ~**58 commands missing** (at audit time)                  |
