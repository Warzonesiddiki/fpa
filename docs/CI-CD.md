...
| 6 Schema/Docs | `scripts/schema-equality-check.mjs` · `scripts/docs-link-check.mjs --strict` · `npm run docs:verify` (index↔files, orphans, 5 states, counts) · `money:ast` · `tokens:check` (no undefined `--color-*` references) | 0 drift; index complete; no float; 0 undefined tokens |
