...
- **Accepted cell input formats (AUDIT-02):** the formula bar and the paste dialog accept Excel-style financial text and normalize it to the exact decimal string before any IPC/audit boundary — plain decimals (`125000.00`), strict 3-digit thousands grouping (`1,250,000.00`), accounting-negative parentheses (`(500.00)` → `-500.00`, also `($1,250)`), one leading currency symbol (`$1,000`), trailing percent as an exact ÷100 (`15%` → `0.15`), and explicit `+`/`-` signs. Zero is always unsigned (`-0.00` → `0.00`). Rejected with the locked `VALUE_INVALID`: scientific notation (`1e3`), broken grouping (`1,25`), double/mixed signs (`--5`, `-+5`), a sign inside parentheses (`(-5)`), interior whitespace, `USD 100`, and any integer part over 12 digits (i64-safe at any scale). The engine `setCell` boundary re-validates and throws `VALUE_INVALID` for anything that is not a plain decimal string (no float, no locale — B3/B18).
- **Keyboard map (Excel parity):** Ctrl/Cmd+Z undo, Ctrl+Shift+Z / Ctrl+Y redo, F2 focus formula bar, Shift+arrows extend selection (app-owned, intercepted in a capture handler); formula-bar Enter applies (with normalization), Escape cancels; plain arrows/Tab/Enter and unmodified letters are left to AG Grid's native navigation (the app intercepts nothing and mutates no model state).
