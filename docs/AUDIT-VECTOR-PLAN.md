...
| AUDIT-20 | Dual-cadence calendar | 🚧 PARTIAL | M1-7 / M3-7 | **Day-count + debt-test slices DONE 2026-09-20:** `src/model/dayCount.ts` (77 tests: `ACT_360`, `ACT_365`, `THIRTY_360_US` + `THIRTY_360_ISDA`) + `src/model/debtSchedule.ts` `periodInterest` (the "debt test: Actual/360 interest exact" + "convention test: 3 methods produce different results" — 1M @ 5.5% over leap-2024 → 55,917 / 55,151 / 55,000, 3 distinct). Remaining: weekly/monthly dual-cadence calendar linkage + native `core/calendar.rs`/`schedule.rs` convention wiring (native, cargo-pending) |
