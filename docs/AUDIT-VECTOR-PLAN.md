...
| AUDIT-20 | Dual-cadence calendar | 🚧 PARTIAL | M1-7 / M3-7 | **Day-count conventions TS engine DONE 2026-09-20** (`src/model/dayCount.ts`, 77 tests: `ACT_360`, `ACT_365`, `THIRTY_360_US` + `THIRTY_360_ISDA`); weekly/monthly dual-cadence linkage + native `core/calendar.rs`/`schedule.rs` convention wiring remain open |
...
| **AUDIT-12** | Treasury / capex persistence | 🚧 PARTIAL | M3-7 / M6-1 | **Depreciation + FCCR TS engine DONE 2026-09-20** (`src/model/depreciation.ts`, 36 exact-decimal tests: SL, DDB with optimal SL switch, MACRS half-year [IRS Pub 946 A-1], FCCR — exact per-year tie-out); SQLite persistence tables (`capital_assets`, etc.) + native `rust_decimal` schedule engine + 13-week DB + SOFR/PIK/undrawn-fee still open (native, cargo-pending) |
