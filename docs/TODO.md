...
- [x] **M4-4** What-if overlay + waterfall + sensitivity tornado + goal seek (S-052) — DONE: native Rust bisection solver and calculation handlers in `src-tauri/src/commands/plan.rs` (`plan.whatif_overlay`, `plan.sensitivity`, `plan.goal_seek`), typed errors (`GOAL_SEEK_NO_CONVERGE`, `SENSITIVITY_OUT_OF_BOUNDS`, `COMPARE_INCOMPATIBLE`), Zod schema and mock bridge, Zustand what-if store (`src/stores/whatif.ts`) with all 5 screen states, accessible S-052 screen (`src/pages/s052-whatif/`) with 3-pane layout, SVG time series/waterfall charts, accessible data tables, tabs for Sensitivity and Goal Seek, `Apply to new Scenario` flow, and lazy route `/app/plan/whatif`.
- [x] **M4-5** Planning cycle manager + close checklist (S-053) — DONE: `cycle.*` lifecycle commands (`cycle.start`, `cycle.task_update`, `cycle.checklist_status`) with milestone transitions, period close task tracking, dependency enforcement (`CYCLE_TASK_BLOCKED`), and duplicate protection (`CYCLE_NAME_DUP`); Rust handler in `src-tauri/src/commands/cycle.rs`; Zod schemas in `src/api/schema.ts`; mock bridge in `src/api/mock.ts`; Zustand store in `src/stores/cycle.ts` with all 5 canonical states; S-053 Planning Cycle page (`src/pages/s053-cycle/`) with milestone ribbon, status board, and close checklist; lazy route `/app/plan/cycle`.
- [x] **M4-6** Input collection loop (S-053) — DONE: `collection.*` distribution and return pipeline (`collection.export`, `collection.import`, `collection.resolve_conflict`); conflict detection (`COLLECTION_CONFLICT`, `COLLECTION_STRUCTURE_CHANGED`); interactive conflict resolution modal in S-053 with radio selection and manual override; driver value collection templates; store, mock, Rust core handlers, and test suites passing.
...
- [x] **M5-3** FVA engine (MAPE/bias/hit) (S-055) — DONE: `fva.get` IPC with exact Decimal arithmetic for MAPE, Bias, and Hit Rate (tolerance $\pm 5\%$), version count threshold ($\ge 3$ versions check returning empty state when $<3$), and `FVA_RESTATEMENT_FLAG` audit detection; Rust handler in `src-tauri/src/commands/fva.rs`; Zod schemas in `src/api/schema.ts`; mock bridge in `src/api/mock.ts`; Zustand store in `src/stores/fva.ts` with 5 canonical states, KPI aggregations, and BU rollup strip; S-055 FVA screen in `src/pages/s055-fva/` with 3-up KPI score cards, accessible formula explanation modal, persistent restatement banner, by-line table with accessible trend chips, and CSV export; route `/app/analyze/fva` registered in `src/router.tsx`; unit, store, and vitest-axe accessibility tests passing.
- [ ] **M5-4** Alerts engine + center + rules (S-056) — 🚧 TS slice DONE (2026-09-05): S-056 page (five states, severity grouping, trigger-chain disclosure, rule manager with audit create), `alerts.list`/`alerts.create_rule` typed contracts + mock mirror (34 tests green); Rust `commands/alerts.rs` + `ALERT_RULE_INVALID` mapping authored, hand-review only (NATIVE-UNVERIFIED — cargo + round-trip pending); dismiss/mute disabled pending Tier-3 catalog rows; KPI-rule evaluation waits M6-4/5.
