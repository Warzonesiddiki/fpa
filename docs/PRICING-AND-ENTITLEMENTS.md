...
5. **One error code, and that is a spec change, not a script edit.** No catalog entry exists for "capability not licensed" (nearest: `LICENSE_EXPIRED` 403, `SESSION_LOCKED` 401 — both wrong meanings). Minting one adds to the 99-code ground truth (the `docs:verify` claim, `DOCS-INDEX.md` row 19 and the traceability note move with it) and must land with a mutation-checked guard, which **KI-015** closed on 2026-09-05 — so this is now unblocked and purely a product call: **D3**.
