...
### KI-014 · `core/money.rs` Largest-Remainder tie order is ascending (spec §4 says descending) · Medium · Open
...
- **Plan:** One-line fix (`rj.cmp(&ri)` / reverse the sort) + keep the deterministic index tie-break, plus a HALF_EVEN tie rule per §4b; land it in the next Rust-touching unit that runs on a cargo-equipped runner, with the two spec vectors asserted exactly (`[12,4,8]`, `[1234.4, 2665.6, 100.0]`). Until then reports must not rely on per-line placement of the residual unit.
