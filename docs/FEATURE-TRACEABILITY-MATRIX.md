...
|---|---|---|---|
| **G1 — Docs machine-green** | Does the doc suite pass every machine check? | ✅ YES (re-verified 2026-09-06) | `npm run docs:verify` → PASS (63 docs indexed, 42 screens, 99 command rows, 99 error codes) · `node scripts/docs-link-check.mjs --strict` → PASS (all relative links resolve, incl. new rows 62–64 DESKTOP-PACKAGING/E2E-TESTING/BENCHMARKS — no FAIL pending) · terminology re-scan → 8 fixed, 0 open violations |
