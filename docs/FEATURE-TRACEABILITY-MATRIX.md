...
> **ZC revision note:** suite was 54 docs/specs at the ZC close (16 supplemental docs — see DOCS-INDEX §39–54); counts corrected to ground truth: **42 screens · 102 typed commands · 56 DB tables · 86 error codes** (2026-09-07 phantom sweep: was 99 — 13 zero-producer rows moved to ERROR-HANDLING §2C as reserved, `UPDATE_FETCH_FAILED` deleted with the updater). **Updated 2026-09-06:** 63 docs/specs + root README = 64 DOCS-INDEX rows after the gap-closure + packaging/bench revisions (DOCS-INDEX §56–64 — COMPETITIVE-ANALYSIS through BENCHMARKS; audit in `DOCUMENTATION-GAP-ANALYSIS.md`) — 97 → 99 typed commands when the shipped `alerts.dismiss` / `alerts.mute_rule` handlers gained their catalog rows, → **102** when `security.pin_setup` / `assumption.waive` (shipped, undocumented) and `pack.validate` (new §17 detailed spec) gained theirs.
...
|---|---|---|---|
| **G1 — Docs machine-green** | Does the doc suite pass every machine check? | ✅ YES (re-verified 2026-09-06) | `npm run docs:verify` → PASS (re-verified 2026-09-07: 63 docs indexed, 42 screens, 102 command rows, 86 error codes + 21 reserved names — §2 phantom-guarded by 7d) · `node scripts/docs-link-check.mjs --strict` → PASS (all relative links resolve, incl. new rows 62–64 DESKTOP-PACKAGING/E2E-TESTING/BENCHMARKS — no FAIL pending) · terminology re-scan → 8 fixed, 0 open violations |
