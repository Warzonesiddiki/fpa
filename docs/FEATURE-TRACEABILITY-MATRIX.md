...
> **ZC revision note:** suite was 54 docs/specs at the ZC close (16 supplemental docs — see DOCS-INDEX §39–54); counts corrected to ground truth: **42 screens · 103 typed commands · 56 DB tables · 87 error codes** (2026-09-07 phantom sweep: was 99 — 13 zero-producer rows moved to ERROR-HANDLING §2C as reserved, `UPDATE_FETCH_FAILED` deleted with the updater). **Updated 2026-09-06:** 63 docs/specs + root README = 64 DOCS-INDEX rows after the gap-closure + packaging/bench revisions (DOCS-INDEX §56–64 — COMPETITIVE-ANALYSIS through BENCHMARKS; audit in `DOCUMENTATION-GAP-ANALYSIS.md`) — 97 → 99 typed commands when the shipped `alerts.dismiss` / `alerts.mute_rule` handlers gained their catalog rows, → **102** when `security.pin_setup` / `assumption.waive` (shipped, undocumented) and `pack.validate` (new §17 detailed spec) gained theirs, → **103** with `company.restore_year` (WS-07 follow-up: the archive mark is restorable).
...
| F-008 Excel/CSV + Driver Data | US-009 | S-030/031 | driver.import, import.* (kind) | driver_values, import_batches | ✅ S-030 Excel/CSV parse-kind hand-off + S-031 finite text/period/sign rules, batch-vs-row validation surface, strict response/mock/store tests, and authored native `UNIT_PERIOD_MISMATCH`/opening gates | ⚠️ M2-5a landed the Opening Balances source tab and its three `OPENING_ALREADY_SET` gates; driver/dimension destination pipelines (`driver_values`/`dimension_values`) are still unbuilt, so their tabs stay disabled and `import.commit` refuses those kinds; S-031 does not invent aggregate/reject actions |
...
| Any doc outside DOCS-INDEX | ✅ PASS — re-verified 2026-09-08 by `npm run docs:verify` (machine gate): 64 docs indexed + root README pointer = 65 DOCS-INDEX rows (65 files on disk in docs/ including DOCS-INDEX.md — row 65 `CI-ADDITIONS.md`); 0 off-index, 0 dangling index rows |
...
| GLOSSARY BANNED `Recalc` vs locked API catalog `model.recalc` command / `recalc` response field / `recalc:done` event | ⚠️ OPEN contradiction | I10 bans BANNED words in identifiers; API-SPEC locks the 99-command catalog. Rename requires a Stage-0 decision + B20 process (cannot be unilaterally changed; Rust rename unverified without cargo). Tracked in TASKBOARD ledger; next catalog revision must resolve |
| CI-CD stage 5/6 script references vs `scripts/` on disk | ❌ Found 2026-08-31 → fixed | `coverage-gate.mjs`, `schema-equality-check.mjs`, `docs-link-check.mjs` did not exist; created (all green in this run); `scripts/docs-index.mjs` ref → actual `scripts/docs-index-gen.mjs` (+ `npm run docs:verify`; DOCS-INDEX header fixed 2026-09-06); `package.json` gained `bench` (PERFORMANCE-REQUIREMENTS §7) |
...
|---|---|---|---|
| **G1 — Docs machine-green** | Does the doc suite pass every machine check? | ✅ YES (re-verified 2026-09-06) | `npm run docs:verify` → PASS (re-verified 2026-09-08: 64 docs indexed, 42 screens, 103 command rows, 87 error codes + 21 reserved names — §2 phantom-guarded by 7d) · `node scripts/docs-link-check.mjs --strict` → PASS (all relative links resolve, incl. new rows 62–64 DESKTOP-PACKAGING/E2E-TESTING/BENCHMARKS — no FAIL pending) · terminology re-scan → 8 fixed, 0 open violations |
