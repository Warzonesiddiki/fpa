...
| F-007 GL Dump Import | US-008 | S-030–S-032 | import.parse/map.save_v1/validate/tieout/commit/rollback/history | import_batches, mapping_templates, mapping_columns, source_files, gl_lines, audit_events | ✅ S-030 12-page-test + store suite; S-031 8-page-test/axe + schema/mock/store tests; Rust mapping transaction/normalization tests authored (native execution CI-only here) | ⚠️ M2-2 reaches Parse→Normalize→Map through real typed IPC. S-032 and M2-3 Validation UI remain; ZIP/progress/cancel/500k bench and `import.history` typed+registered read path remain explicit gates |
| F-008 Excel/CSV + Driver Data | US-009 | S-030/031 | driver.import, import.* (kind) | driver_values, import_batches | ✅ S-030 Excel/CSV parse-kind hand-off + S-031 finite text/period/sign rule tests + existing schema/mock/core tests | ⚠️ driver/opening/dimension source tabs and full same-pipeline UI remain M2-5 |
...
| F-010 Vault & Reconciliation | US-011 | S-034, S-030 | reconcile.run, import.history | source_files | ✅ planned unit+integration | ⚠️ Source Vault persistence, `import.history`, and `reconcile.run` runtime paths are absent; S-030 gates them visibly |
| F-011 Mapping Management | US-012 | S-031 | import.map.save_v1, validate | mapping_templates, mapping_columns, audit_events | ✅ strict schema/result + mock parity + Company-scoped store tests; 8 S-031 state/a11y tests; Rust validation/normalization/version/audit-rollback tests authored | ✅ M2-2: route/search/S-030 hand-off + native invoke are implemented. Latest body materialized; immutable full definitions ride audit before/after. ⚠️ no catalogued mapping-list/load/history command, so browsing is visibly gated; native Rust gates blocked locally: `cargo`, `rustc`, `rustfmt`, and `clippy-driver` are unavailable |
