...
| Any doc outside DOCS-INDEX | ✅ PASS — re-verified 2026-09-06 by `npm run docs:verify` (machine gate): 63 docs indexed + root README pointer = 64 DOCS-INDEX rows (64 files on disk in docs/ including DOCS-INDEX.md); 0 off-index, 0 dangling index rows |
...
|---|---|---|---|
| **G1 — Docs machine-green** | Does the doc suite pass every machine check? | ✅ YES (re-verified 2026-09-06) | `npm run docs:verify` → PASS (re-verified 2026-09-07: 63 docs indexed, 42 screens, 102 command rows, 87 error codes + 21 reserved names — §2 phantom-guarded by 7d) · `node scripts/docs-link-check.mjs --strict` → PASS (all relative links resolve, incl. new rows 62–64 DESKTOP-PACKAGING/E2E-TESTING/BENCHMARKS — no FAIL pending) · terminology re-scan → 8 fixed, 0 open violations |
