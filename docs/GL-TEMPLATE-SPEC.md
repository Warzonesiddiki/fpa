...
| Account missing from COA during validation | HARD `MAP_ACCOUNT_AMBIGUOUS` with row detail `ACCOUNT_MISSING`; S-031 offers source/mapping correction and re-validation only. It has no account-create or per-row remap action. **Added 2026-09-26 (AUDIT-07 TS slice):** S-031 also shows the *closest COA matches* per `ACCOUNT_MISSING` row — Jaro-Winkler similarity computed client-side (`src/model/jaroWinkler.ts`) over the catalogued `coa.list` data, top 3 at threshold 0.85 — advisory only: never auto-applied, the hard gate and remediation surface are unchanged (native `account_mappings` table + `coa.create` = Tier-3 follow-up). |
