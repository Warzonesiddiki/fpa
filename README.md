...
npm run check            # full gate: lint + typecheck + fmt:check + vitest + coverage (main ≥85/80, critical ≥95/90) + schema-equality + docs-links + docs:verify + packs + money:ast + tokens + ipc:casing + security
...
| `npm run check` | Full gate (lint + typecheck + fmt:check + vitest + coverage main ≥85/80 + critical ≥95/90 + schema-equality-check + docs-link-check + docs:verify + packs:validate + money:ast + tokens:check + ipc:casing + security:scan) |
